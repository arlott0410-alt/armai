...
async function getConversationChannelInfo(
  ...
  const { data, error } = await supabase
    .from('conversations')
    .select('merchant_id, channel_type, external_account_id, external_customer_id, page_id, customer_psid')
 ...
 */
export async function sendChannelMessage(params: SendChannelMessageParams): Promise<SendChannelMessageResult> {
  ...
  if (info.channelType === 'whatsapp') {
    const withinSession = await isWhatsAppSessionActive(supabase, info.merchantId, info.externalCustomerId);
    if (!withinSession) {
      return {
        sent: false,
